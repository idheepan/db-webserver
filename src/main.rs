...
use db_webserver::{
    models::{SensorReading, NewSensorReading},
    schema::data,
...
async fn retrieve_some(
    ...
    let start_time = DateTime::parse_from_rfc3339(startts.as_str()).unwrap();
    let end_time = DateTime::parse_from_rfc3339(endts.as_str()).unwrap();

    connection
    .run(move |c| data::table.filter(data::ts.between(start_time, end_time)).load(c))
    .await
    .map(Json)
    .map_err(|_e| {
        NoContent
